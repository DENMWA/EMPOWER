...
export function AvailabilityDocumentWorkflow({ staffInviteId, staffName, onPublished }: { staffInviteId: string; staffName: string; onPublished: (records: StaffAvailability[]) => void }) {
  ...
  async function parseForm() {
    if (!file || !staffInviteId) return setMessage("Choose staff and upload the completed availability form.");
  ...
  return (
    <Card className="xl:col-span-2">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-[1fr_auto]">
        <label className="grid gap-1 text-sm font-medium text-slate-700">Completed employee form<input type="file" accept="application/pdf,.pdf,image/jpeg,image/png,image/webp,.jpg,.jpeg,.png,.webp" onChange={(event) => setFile(event.target.files?.[0] || null)} className="min-h-11 rounded-md border border-slate-300 bg-white px-3 py-2" /></label>
