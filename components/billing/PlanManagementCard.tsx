...
export function PlanManagementCard() {
  ...
  async function openBillingEndpoint(endpoint: "checkout" | "portal") {
    ...
    try {
      ...
      if (!response.ok || !result.url) {
        ...
        setNeedsEmailConfirmation(result.code === "email_unconfirmed");
        setBusyAction("");
        return;
      }
  ...
  async function resendConfirmation() {
    ...
    try {
      ...
      setBillingMessage(result.ok ? `Confirmation email sent to ${result.email}. Click the link in that email, then try again.` : result.error || "Could not send the confirmation email.");
    } catch {
      setBillingMessage("The email service is temporarily unavailable. Try again shortly.");
    } finally {
  ...
  return (
    <Card className="overflow-hidden border-slate-200 p-0">
      ...
      {billingMessage ? (
        <div aria-live="polite" className="mt-4 rounded-md bg-slate-100 px-3 py-2 text-sm font-semibold text-slate-700">
          <p>{billingMessage}</p>
          {needsEmailConfirmation ? (
            <button type="button" disabled={resendingConfirmation} onClick={resendConfirmation} className="mt-2 inline-flex min-h-9 items-center rounded-md border border-slate-300 bg-white px-3 text-xs font-semibold text-ink hover:border-teal-400 disabled:opacity-60">
