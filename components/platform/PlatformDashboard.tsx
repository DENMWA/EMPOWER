...
function LiveOrganisationTable({ data, operations, onRefresh, compact = false }: { data: LivePlatformSummary; operations: LivePlatformOperations; onRefresh: () => void; compact?: boolean }) {
  ...
  const recentProviders = [...data.organisations].filter((org) => daysSince(org.signedUpAt) <= 30).sort((a, b) => b.signedUpAt.localeCompare(a.signedUpAt)).slice(0, 6);
...
function formatPlatformDateTime(value: string) {
  if (!value) return "Time not recorded";
  const date = new Date(value);
  return Number.isNaN(date.getTime()) ? "Time not recorded" : date.toLocaleString("en-AU", { dateStyle: "medium", timeStyle: "short" });
}
...
function registrationAgeLabel(value: string) {
  const days = daysSince(value);
  if (!Number.isFinite(days)) return "Registration date unavailable";
