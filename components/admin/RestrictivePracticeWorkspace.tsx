...
import { getRestrictivePracticeAuthorisations, getRestrictivePracticeUses, restrictivePracticeTypes, saveRestrictivePracticeAuthorisation, saveRestrictivePracticeUse, type RestrictivePracticeAuthorisation, type RestrictivePracticeUse } from "@/lib/restrictive-practice-records";
...
function Authorisations({ clients, houses, records, onSaved }: { clients: ClientRecord[]; houses: HouseRecord[]; records: RestrictivePracticeAuthorisation[]; onSaved: (message: string) => void }) {
  const [form, setForm] = useState<RestrictivePracticeAuthorisation>({ id: uid(), participantId: "", houseId: "", practiceType: "Environmental restraint", practiceName: "", behaviourSupportPlan: "", authorisingBody: "", authorisationReference: "", startsOn: today(), expiresOn: "", conditions: "", maximumDurationMinutes: null, maximumFrequency: "", status: "Active" });
  ...
  async function save() {
    if (!form.participantId || !form.practiceName || !form.authorisationReference || !form.expiresOn) return onSaved("Complete the client, authorised practice, reference, and expiry date.");
  ...
  return <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(320px,.8fr)]">
    <Card><h2 className="text-xl font-bold text-ink">Add authorisation</h2><div className="mt-5 grid gap-4 sm:grid-cols-2">
      ...
      <Select label="Practice type" value={form.practiceType} onChange={(value) => update("practiceType", value as RestrictivePracticeAuthorisation["practiceType"])} options={restrictivePracticeTypes.map((item) => [item,item])} />
      ...
      <Input label="Authorisation reference" value={form.authorisationReference} onChange={(value) => update("authorisationReference", value)} />
...
function UseLog({ clients, houses, authorisations, uses, onSaved }: { clients: ClientRecord[]; houses: HouseRecord[]; authorisations: RestrictivePracticeAuthorisation[]; uses: RestrictivePracticeUse[]; onSaved: (message: string) => void }) {
  const [form, setForm] = useState<RestrictivePracticeUse>({ id: uid(), authorisationId: "", participantId: "", houseId: "", usedAt: localNow(), endedAt: "", triggerContext: "", alternativesAttempted: "", implementation: "", participantResponse: "", monitoring: "", recoverySupport: "", injuryOrHarm: false, injurySummary: "", matchedAuthorisation: true, varianceDetails: "", staffNames: "", notifications: "", status: "Draft" });
  const client = clients.find((item) => item.id === form.participantId);
  const availableAuthorisations = authorisations.filter((item) => item.participantId === form.participantId && item.status === "Active");
  const availableHouses = client ? getHousesForClient(houses, client) : [];
  ...
  return <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(320px,.8fr)]"><Card><h2 className="text-xl font-bold text-ink">Record a use</h2><div className="mt-5 grid gap-4 sm:grid-cols-2">
    ...
    <Select label="House / service" value={form.houseId} onChange={(value) => update("houseId", value)} options={availableHouses.map((item) => [item.id,item.name])} placeholder="Choose service" />
    <Select label="Authorisation" value={form.authorisationId} onChange={(value) => update("authorisationId", value)} options={availableAuthorisations.map((item) => [item.id,`${item.practiceType}: ${item.practiceName}`])} placeholder="No matching authorisation" />
  ...
  {!form.matchedAuthorisation ? <TextArea label="Variance details" value={form.varianceDetails} onChange={(value) => update("varianceDetails", value)} /> : null}{form.injuryOrHarm ? <TextArea label="Injury or harm details" value={form.injurySummary} onChange={(value) => update("injurySummary", value)} /> : null}
  ...
  <Card><h2 className="text-xl font-bold text-ink">Use log</h2><div className="mt-4 space-y-3">{uses.length ? uses.map((record) => { const needsIncident = record.injuryOrHarm || !record.matchedAuthorisation || !record.authorisationId; return <div key={record.id} className="rounded-md border border-slate-200 p-4"><div className="flex justify-between gap-3"><div><p className="font-bold text-ink">{clients.find((item) => item.id === record.participantId)?.name || "Client"}</p><p className="mt-1 text-sm text-slate-600">{new Date(record.usedAt).toLocaleString("en-AU")}</p></div><StatusBadge label={record.status} tone={record.status === "Reviewed" ? "green" : "amber"} /></div><p className="mt-3 line-clamp-3 text-sm leading-6 text-slate-700">{record.implementation}</p>{needsIncident && !record.linkedIncidentId ? <Link href={incidentHref(record)} className="mt-3 inline-flex min-h-10 items-center gap-2 rounded-md border border-red-200 bg-red-50 px-3 text-sm font-semibold text-red-800"><FileWarning size={16} />Create linked incident</Link> : null}</div>; }) : <Empty text="No use records submitted." />}</div></Card></div>;
}
