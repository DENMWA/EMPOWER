...
import { enrollTotpMfa, getCurrentAuthStatus, getMfaFactors, verifyTotpMfa } from "@/lib/supabase-auth";
...
export function MfaSecurityPanel() {
  ...
  useEffect(() => {
    ...
    void getMfaFactors().then(async (result) => {
      const verified = result.factors.find((factor) => factor.factor_type === "totp" && factor.status === "verified");
      if (verified) setFactorId(verified.id);
      else await createEnrollment();
      setBusy(false);
    });
  }, []);
  ...
  async function createEnrollment() {
    ...
    if (result.error || !result.data) setMessage(result.error || "Authenticator setup could not be started.");
      ...
      setEnrollment({ qrCode: result.data.totp.qr_code, secret: result.data.totp.secret });
    }
    setBusy(false);
  }
  ...
  return (
    <Card className="mx-auto max-w-lg border-teal-200 shadow-lift">
      ...
      {enrollment ? <div className="mt-5 grid justify-items-center gap-3 rounded-md border border-slate-200 p-4 text-center"><Image src={enrollment.qrCode} alt="Authenticator setup QR code" width={192} height={192} unoptimized /><p className="text-xs text-slate-600">Cannot scan? Enter this setup key:</p><code className="break-all text-sm font-semibold text-ink">{enrollment.secret}</code></div> : null}
