    ...
    <Card>
      ...
      <div className="mt-4 overflow-x-auto">
        <table className="w-full min-w-[720px] text-left text-sm">
          <thead><tr className="border-b"><th className="py-3">Feature</th><th>Sole</th><th>Team</th><th>Growth</th><th>Enterprise</th></tr></thead>
          <tbody>{rows.map((row) => <tr key={row[0]} className="border-b border-slate-100">{row.map((cell) => <td key={cell} className="py-3 pr-4">{cell}</td>)}</tr>)}</tbody>
        </table>
      </div>
    </Card>
  );
