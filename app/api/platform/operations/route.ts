...
export async function GET(request: Request) {
  ...
  const [security, support, usage, observations, audits, snapshots, ndisMatches] = await Promise.all([
    ...
    read(db, "ndis_invoice_match_events?select=organisation_id,outcome,match_source,failure_category,selected_support_item_number,selected_price,confidence,candidate_count,occurred_at&order=occurred_at.desc&limit=5000")
  ]);
  ...
  return NextResponse.json({
    ...
    ndisMatchEvents: ndisMatches.rows,
    availability: {
