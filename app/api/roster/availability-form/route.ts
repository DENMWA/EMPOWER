...
export async function POST(request: Request) {
  ...
  const body = await request.json() as { staffInviteId?: string };
  if (!body.staffInviteId) return NextResponse.json({ error: "Choose a staff member." }, { status: 400 });
  ...
  if (!staff[0]) return NextResponse.json({ error: "Staff member was not found in this organisation." }, { status: 404 });
  const pdf = createAvailabilityFormPdf(staff[0].name, organisations[0]?.name || "EmpowerNotes");
  const safeName = staff[0].name.replace(/[^a-z0-9]+/gi, "-").replace(/^-|-$/g, "").toLowerCase() || "employee";
