...
import { getPublicSeoPage, publicSeoPages } from "@/lib/public-seo-pages";

const appUrl = process.env.NEXT_PUBLIC_APP_URL ?? "https://www.empowernotes.org";

type FeaturePageProps = { params: { slug: string } };

export function generateStaticParams() {
  return publicSeoPages.map((page) => ({ slug: page.slug }));
}

export function generateMetadata({ params }: FeaturePageProps): Metadata {
  const page = getPublicSeoPage(params.slug);
  if (!page) return {};
  return {
    ...
    "@graph": [
      {
        ...
        audience: page.audience.map((audience) => ({ "@type": "Audience", audienceType: audience })),
        description: page.description,
        url: `${appUrl}/features/${page.slug}`,
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 3, name: page.title, item: `${appUrl}/features/${page.slug}` }
        ]
      }
    ]
  };
    ...
    <>
      ...
      <Section className="space-y-8">
          ...
          <aside className="space-y-5">
            ...
            <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-soft">
              ...
              <ul className="mt-4 space-y-2 text-sm leading-6 text-slate-700">
                {page.audience.map((audience) => <li key={audience}>- {audience}</li>)}
              </ul>
            </div>
          </aside>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 rounded-lg border border-teal-200 bg-teal-50 p-5">
          <div>
            <p className="font-bold text-ink">Want the wider platform view?</p>
            <p className="mt-1 text-sm text-slate-600">See how notes, incidents, rostering, records and billing work together.</p>
          </div>
          <Link href="/ndis-software-australia" className="inline-flex min-h-11 items-center gap-2 rounded-md bg-ink px-4 text-sm font-semibold text-white">
            View NDIS operations software <ArrowRight size={16} aria-hidden="true" />
          </Link>
        </div>
      </Section>
    </>
