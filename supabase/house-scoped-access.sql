...
returns text[] language sql immutable as $$
  select case selected_role
    ...
    when 'operations_manager' then array['participants.view_basic','participants.view_support','notes.view','notes.review','incidents.view','incidents.review','incidents.manage_followup','handover.view','rostering.view','rostering.manage','rostering.assign_staff','staff.view','staff.manage','documents.view','reports.view','reports.export','house.dashboard.view','organisation.dashboard.view']
    ...
    else array['participants.view_basic','participants.view_support','participants.view_sensitive','notes.create','notes.view','notes.review','notes.approve','incidents.create','incidents.view','incidents.review','incidents.manage_followup','meals.create','meals.view','handover.view','handover.create','rostering.view','rostering.manage','rostering.assign_staff','staff.view','staff.manage','documents.view','documents.manage','billing.view','billing.manage','billing.approve','budgets.view','budgets.manage','house.dashboard.view','organisation.dashboard.view','service_agreements.view','service_agreements.manage','reports.view','reports.export','settings.view','settings.manage']
  end;
$$;
...
drop policy if exists "plans visible by participant access" on public.participant_plans;
create policy "plans visible by active participant scope" on public.participant_plans for select to authenticated
...
create policy "participant goals visible by active scope" on public.participant_goals for select to authenticated
...
create policy "goal evidence visible by active scope" on public.goal_evidence for select to authenticated
using (organisation_id = public.current_user_organisation_id() and private.current_user_has_permission('participants.view_support') and private.current_user_can_access_participant(participant_id));
