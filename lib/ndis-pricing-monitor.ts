...
const OFFICIAL_PAGE = "https://www.ndis.gov.au/providers/pricing-and-payments/pricing/pricing-arrangements";
...
export async function checkOfficialNdisPricing(config: DbConfig) {
  ...
  if (previous?.detected_checksum === checksum) {
  ...
  if (!/\.csv$/i.test(sourceUrl.pathname)) {
    return saveMonitor(config, { checkedAt, pageChecksum, status: "review_required", alertStatus: "open", detail: `${filename} is a new official release, but it is not a machine-readable CSV. Download the official catalogue CSV and use the manual import fallback.`, detectedDownloadUrl: sourceUrl.toString(), detectedFilename: filename, detectedChecksum: checksum, draftVersionId: null });
  }

  const effectiveFrom = inferEffectiveDate(filename, html);
  const imported = await importCsvDraft(config, source, filename, sourceUrl.toString(), effectiveFrom);
...
async function importCsvDraft(config: DbConfig, source: Buffer, filename: string, sourceUrl: string, effectiveFrom: string) {
  const rows = parseCsv(source.toString("utf8")); const headers = rows[0]?.map(normaliseHeader) || []; const columns = resolveColumns(headers);
  if (columns.code < 0 || columns.name < 0 || columns.unit < 0 || !columns.priceColumns.length) throw new Error("The official CSV format changed and needs developer review.");
  ...
  await fetch(`${config.url}/rest/v1/ndis_pricing_import_jobs`, { method: "POST", headers: headersDb, body: JSON.stringify({ organisation_id: null, import_method: "automatic_official_ndis_csv", source_url: sourceUrl, source_filename: filename, status: "completed", started_at: new Date().toISOString(), completed_at: new Date().toISOString(), result_pricing_version_id: versionId }) });
...
function parseCsv(value:string){const rows:string[][]=[];let row:string[]=[],v="",q=false;for(let i=0;i<value.length;i++){const c=value[i],n=value[i+1];if(c==='"'&&q&&n==='"'){v+='"';i++;}else if(c==='"')q=!q;else if(c===","&&!q){row.push(v);v="";}else if((c==="\n"||c==="\r")&&!q){if(c==="\r"&&n==="\n")i++;row.push(v);if(row.some(x=>x.trim()))rows.push(row);row=[];v="";}else v+=c;}row.push(v);if(row.some(x=>x.trim()))rows.push(row);return rows;}
