...
export const publicCapabilities = [
  { id: "documentation-health-check", name: "Free NDIS documentation health check", description: "A public, de-identified checker scores sample notes, incident reports, audit evidence or billing records for clarity, evidence signals and review gaps.", url: "/ndis-documentation-health-check" },
...
export function getLlmsText() {
  ...
  const landingPages = publicLandingPages.map((page) => `- [${page.metaTitle}](${publicProductProfile.url}/${page.slug}): ${page.description}`).join("\n");
...
export function getAiManifest() {
  return {
    ...
    documentation: [`${publicProductProfile.url}/ndis-software-australia`, `${publicProductProfile.url}/ndis-documentation-health-check`, ...publicLandingPages.map((page) => `${publicProductProfile.url}/${page.slug}`), `${publicProductProfile.url}/features`, ...publicSeoPages.map((page) => `${publicProductProfile.url}/features/${page.slug}`), `${publicProductProfile.url}/ai-progress-notes`, `${publicProductProfile.url}/legal/privacy`],
